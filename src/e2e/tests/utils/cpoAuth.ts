...
import { cpoUsername, cpoPassword } from '../../env';
...
export async function cpoLogin(page: Page) {
  try {
    ...
    await page.locator('#user').fill(cpoUsername);

    // Enter the CPO password
    await page.getByLabel('Password').click();
    await page.getByLabel('Password').fill(cpoPassword);
...
export async function cpoLogout(page: Page) {
  try {
