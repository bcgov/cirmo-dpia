...
class submitPage extends Page {
    ...
    get LeadNamelabel () {
        return $("//label[contains(text(),'Initiative lead name')]/following-sibling::input");
    }
    get LeadTitlelabel () {
        return $("//label[contains(text(),'Initiative lead title')]/following-sibling::input");
    }
    get LeadEmaillabel () {
        return $("//label[contains(text(),'Initiative lead email')]/following-sibling::input");
    }
    get initiativeText () {
        return $("//h3[contains(text(),'Initiative Details')]/../div//textarea");
    }
    get scopeText () {
        return $("//section/div/p/strong[contains(text(),'What is the scope of the PIA?')]/../..//textarea");
    }
    get dataText () {
        return $("//section/div/p/strong[contains(text(),'What are the data or information elements involved in your initiative?')]/../..//textarea");
    }
    get labelsNo () {
    ...
    async setScenario () {
        ...
        var date = new Date(); 
        scenario = date.getFullYear() + ("0" + (date.getMonth() + 1)).slice(-2) + ("0" + date.getDate()).slice(-2) + ("0" + date.getHours() ).slice(-2) + ("0" + date.getMinutes()).slice(-2) + ("0" + date.getSeconds()).slice(-2)
        }
    ...
    async enterValue (value) {
        ...
        if (value=='Initiative Details')
        {
            ...
            await this.initiativeText.setValue("Describe your initiative in enough detail that a reader who knows nothing about your work will understand the purpose of your initiative and who your partners and other stakeholders are. Describe what you’re doing, how it works, who is involved and when or how long your initiative runs.123#$%"); 
        }
        if (value=='Scope Text')
        {
            await this.scopeText.waitForExist();
            await this.scopeText.setValue("Your initiative might be part of a larger one or might be rolled out in phases. What part of the initiative is covered by this PIA? What is out of scope of this PIA?123#$%");
        }
        if (value=='Data Text')
        {
