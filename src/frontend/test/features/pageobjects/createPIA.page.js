...
class createPage extends Page {
    ...
    async intakeForm () {
      ...
      {
        const ExpUrl = 'https://localhost:8080/pia/new/intake';
        await expect(browser).toHaveUrl(ExpUrl);
      }
      else
      {
        await expect("Intake URL not equal to dev,test or local").toEqual("Intake URL should be dev,test or local");
      }
  ...
  async PPQForm () {
    ...
    {
      const ExpUrl = 'https://localhost:8080/ppq-form';
      await expect(browser).toHaveUrl(ExpUrl);
    }
    else
    {
      await expect("PPQ URL not equal to dev,test or local").toEqual("PPQ URL should be dev,test or local");
    }
