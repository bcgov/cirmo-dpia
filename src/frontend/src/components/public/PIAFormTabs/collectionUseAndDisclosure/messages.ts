// This object contains all of the text values for the PPQ Connect component
export default {
  Headings: {
    Title: {
      en: `Collection, use and disclosure`,
    },
    Subtitle: {
      en: `This section will help you to identify the legal authority for collecting, using, and disclosing personal information and to confirm that all personal information elements are necessary for the purpose of the initiative.`,
    },
  },

  WorkThroughDetails: {
    Title: {
      en: `Step-by-step walkthrough`,
    },
    SubTitle: {
      en: `Describe the way personal information moves through your initiative step by step as if you were explaining it to someone who does not know about your initiative.`,
    },
    ColumnDrafterInput: {
      en: `Describe the step`,
    },
    ColumnMpoInput: {
      en: `Collection, use, disclosure (for MPO use only)`,
    },
    ColumnFoippaInput: {
      en: `FOIPPA authority (for MPO use only)`,
    },
    ColumnOtherInput: {
      en: `Other legal authority (for MPO use only)`,
    },
  },
  CollectionNotice: {
    Title: {
      en: `Collection notice`,
    },
    DrafterInput: {
      Title: {
        PartOne: {
          en: `Review the `,
        },
        LinkText: {
          en: ` sample collection notice`,
        },
        PartThree: {
          en: ` and write your collection notice below.`,
        },
      },
      Link: {
        en: `https://www2.gov.bc.ca/gov/content/governments/services-for-government/information-management-technology/privacy/privacy-impact-assessments/guidance-for-privacy-impact-assessments#q-6`,
      },
      Description: {
        en: `If you are collecting personal information directly from the individual the information is about, FOIPPA requires that you provide a collection notice (except in limited circumstances). If you believe there is a reason for an exception, contact your MPO.`,
      },
    },
    MpoInput: {
      Title: {
        en: `For MPO use only: if applicable, list the exception to a collection notice.`,
      },
    },
  },
};
