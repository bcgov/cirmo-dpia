export const routes = {
  PPQ_LANDING_PAGE: '/ppq',
  PPQ_FORM: '/ppq-form',
  PPQ_CONNECT_WITH_MPO: '/ppq-connect',
  PIA_NEW: '/pia/new/intake',
  PIA_VIEW: '/pia/:id',

  PIA_PRINT_PREVIEW: '/pia/:id/preview',

  PIA_INTAKE_EDIT: '/pia/:id/intake/edit',
  PIA_INTAKE_VIEW: '/pia/:id/intake/view',

  PIA_NEXT_STEPS_EDIT: '/pia/:id/nextSteps/edit',

  PIA_DISCLOSURE_EDIT: '/pia/:id/collectionUseAndDisclosure/edit',
  PIA_DISCLOSURE_VIEW: '/pia/:id/collectionUseAndDisclosure/view',

  PIA_STORING_PERSONAL_INFORMATION_EDIT:
    '/pia/:id/storingPersonalInformation/edit',
  PIA_STORING_PERSONAL_INFORMATION_VIEW:
    '/pia/:id/storingPersonalInformation/view',

  PIA_SECURITY_OF_PERSONAL_INFORMATION_EDIT:
    '/pia/:id/securityOfPersonalInformation/edit',
  PIA_SECURITY_OF_PERSONAL_INFORMATION_VIEW:
    '/pia/:id/securityOfPersonalInformation/view',

  PIA_ACCURACY_CORRECTION_AND_RETENTION_EDIT:
    '/pia/:id/accuracyCorrectionAndRetention/edit',
  PIA_ACCURACY_CORRECTION_AND_RETENTION_VIEW:
    '/pia/:id/accuracyCorrectionAndRetention/view',

  PIA_AGREEMENTS_AND_INFORMATION_BANK_EDIT:
    '/pia/:id/agreementsAndInformationBanks/edit',
  PIA_AGREEMENTS_AND_INFORMATION_BANK_VIEW:
    '/pia/:id/agreementsAndInformationBanks/view',

  PIA_ADDITIONAL_RISKS_EDIT: '/pia/:id/additionalRisks/edit',
  PIA_ADDITIONAL_RISKS_VIEW: '/pia/:id/additionalRisks/view',

  PIA_PPQ_EDIT: '/pia/:id/ppq/edit',
  PIA_PPQ_VIEW: '/pia/:id/ppq/view',

  PIA_REVIEW: '/pia/:id/review/view',

  PIA_RESULT: '/pia/result',
  PIA_LIST: '/pia/list',
  NOT_FOUND: '/not-found',
  NOT_AUTHORIZED: '/not-authorized',
};
