...
const devDependencies =
  Object.entries(packageJson.devDependencies) ?? undefined;
...
(async () => {
  ...
  const devDependenciesUpToDate =
    devDependencies.length - devDependencyResults.length;
  const totalDependenciesUpToDate =
    dependenciesUpToDate + devDependenciesUpToDate;

  const totalPercentageUpToDate = Math.round(
    (totalDependenciesUpToDate /
  ...
  if (devDependencyResults.length === 0) {
    ...
    console.log(
      `${attention} - ${devDependencyResults.length} Dev dependencies are out-of-date.`
    );
  }
  ...
  if (dependencyResults.length > 0) {
    ...
    const patchDependencies = dependencyResults.filter(
      (dep) => dep.versionChange === "Patch"
    );
    const minorDependencies = dependencyResults.filter(
      (dep) => dep.versionChange === "Minor"
    );
    const majorDependencies = dependencyResults.filter(
      (dep) => dep.versionChange === "Major"
    );
  ...
  if (devDependencyResults.length > 0) {
    ...
    // Seperate by patch, minor and major.
    const patchDependencies = devDependencyResults.filter(
      (dep) => dep.versionChange === "Patch"
    );
    const minorDependencies = devDependencyResults.filter(
      (dep) => dep.versionChange === "Minor"
    );
    const majorDependencies = devDependencyResults.filter(
      (dep) => dep.versionChange === "Major"
    );
